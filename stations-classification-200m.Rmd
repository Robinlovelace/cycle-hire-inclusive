...
stations_200m_buffer = stations %>% 
  st_transform(27700) %>% 
  st_buffer(200) %>% 
  st_transform(4326)

mapview::mapview(stations_200m_buffer)
...
lnd_osm_near_stations = lnd_osm_centroids[stations_200m_buffer, ]
# mapview::mapview(stations_200m_buffer) + mapview::mapview(lnd_osm_near_stations)
plot(sf::st_geometry(lnd_osm_near_stations))

stations_pop = aggregate(lnd_osm_near_stations["pop_per_building"], stations_200m_buffer, sum)
...
saveRDS(stations, "stations_pop_200m.Rds")
...
```{r}
stations = readRDS("stations_pop_200m.Rds")
names(stations)
head(stations$)
...
p
ggsave("figures/income-decile-am-pm-boxplot-200m.png", p, width = 9, height = 7)
...
write_sf(stations, "stations_imd_pop_200m.geojson", delete_dsn = TRUE)
