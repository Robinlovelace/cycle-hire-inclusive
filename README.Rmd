...
# title: "Can cycle hire schemes support transport equality? An evaluation of almost a decade of trips"
...
# citr::tidy_bib_file(rmd_file = "README.Rmd", messy_bibliography = "~/data/allrefs.bib", file = "references.bib") # works but slow
```

```{r, echo=FALSE, eval=FALSE, engine='bash'}
cp README.pdf ~/Dropbox/paper-submissions/bike-hire-inequalities.pdf
cp README.docx ~/Dropbox/paper-submissions/bike-hire-inequalities.docx
```
...
Data on docking stations was provided by the Consumer Data Research Centre, which provided a '.csv' file containing 1,130 rows with 19 variables including docking station ID, name, location coordinates and its opening date.
The other key dataset used in this study was Origin-Destination (OD) trip data, published by [Transport for London](https://cycling.data.tfl.gov.uk) and made easily accessible via the [bikedata](https://github.com/ropensci/bikedata) R package [@padgham_bikedata:_2017]. Both datasets can be linked via a docking station ID. In the _trips_ table, the origin and destination docking station and corresponding timestamps is recorded. The _docking station_ table contains IDs, names and geographic coordinates for each docking station. For each trip in the _trips_ table, the origin and destination docking station is linked to the _docking station_ table via the _docking station id_ variable.
After cleaning the database to remove duplicate trips and ensure that records omitted from the bikedata package (version `0.2.3`),
...
```{r rank-size, out.width="60%", fig.cap="Rank-size distribution of OD pairs for all cycled trips on London Cycle Hire Scheme", eval=FALSE}
knitr::include_graphics("figures/rank_size.png")
```


```{r stations-powerlaw, eval=TRUE, fig.cap="Cumulative distribution function plot, showing the rank of station popularity (x axis) in relation to the log of trip counts originating in different stations.", out.width="100%", fig.height=4}
# stations = sf::read_sf("stations_imd_pop.geojson")
# stations = stations %>% filter(!is.na(total_n_trips_start), total_n_trips_start > 1000)
# stations$ln_n_trips = log10(stations$total_n_trips_start)
# stations$rank = (rank(stations$total_n_trips_start))
# p1 = ggplot(stations) + geom_point(aes(rank, ln_n_trips))
# stations_resi = sf::read_sf("stations_residential.geojson")
# stations$residential = stations$id %in% stations_resi$id
# stations$ln_n_trips = log10(stations$total_n_trips_start)
# stations$rank = (rank(stations$total_n_trips_start))
# # ggplot(stations) + geom_point(aes(rank, ln_n_trips, colour = residential))
# # plot(ln_rank, ln_n_trips)
# library(poweRlaw)
# m_bl = conpl$new(stations$total_n_trips_start)
...
# m_bl_ln = conlnorm$new(stations$total_n_trips_start)
# est = estimate_xmin(m_bl_ln)
# m_bl_ln$setXmin(est)
# lines(m_bl_ln, col=3, lwd=2)
# knitr::include_graphics("README_files/figure-gfm/stations-powerlaw-1.png")
```
...
# file.edit("stations-classification.Rmd") # see stations-classification.Rmd for source code
knitr::include_graphics(c("figures/facet-imd.png", "figures/stations-imd-facet-4-grey.png"))
```
...
# file.edit("stations-classification.Rmd") # see stations-classification.Rmd for source code
knitr::include_graphics("figures/income-decile-am-pm-boxplot.png")
```
...
knitr::include_graphics("figures/daily_hires_station_imd.png")
```
...
knitr::include_graphics("figures/trip_types_by_borough.png")
```
...
However, there are some limitations to the study that should be considered before generalising the findings.

The first limitation relates to the measure of income.
Although the geographic resolution of analysis was relatively high (each LSOA zone contains less than 2000 people), the study lacks demographic data at level of individual bikeshare users.
We know very little about the income (and other) characteristics of users *compared with the demographic profile of their local area*.
...
Likewise, are the bikeshare users (especially those living in prosperous areas) on relatively low incomes, reflecting 'micro-segregation' [@keddie_market_2010]?
