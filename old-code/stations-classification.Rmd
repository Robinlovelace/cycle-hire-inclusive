...
stations_buff = sf::read_sf("stations_buff.geojson")
stations = readRDS("bikelocations_sf.Rds")
stations$id = as.character(stations$ucl_id)
stations_2019 = stations %>% filter(year == 2019)
plot(stations_buff$geometry)
plot(stations_2019["initial_size"], add = TRUE)
```
...
plot(lsoa)
station_lsoa = st_join(stations, lsoa)
...
ggsave("clean-figures/stations-imd-facet-4-grey.png")
...
#   summarise(n = n()) %>% 
#   ggplot(aes(`Income decile`)) +
#   geom_bar(aes(y = (`Income decile`)/n), stat = "identity", fill = "blue") + 
#           scale_y_continuous(labels=scales::percent) +
#   ylab("Relative frequencies") +
#   facet_wrap(~year) +
#   ggthemes::theme_clean()

stations_income_yearly = station_lsoa %>% 
  group_by(year) %>% 
  summarise(
  ...
  st_drop_geometry()
stations_income_yearly
ggsave("clean-figures/stations-imd-facet-4.png")
```
...
stations_ucl_lsoa = readRDS("stations-clean.Rds")
stations_ucl_lsoa = st_join(stations_ucl_clean, lsoa)
plot(stations_ucl_lsoa["Income decile"])
stations_ucl_lsoa = stations_ucl_lsoa %>% rename(id = ucl_id) %>% 
  mutate(id = as.character(id))
saveRDS(stations_ucl_lsoa, "stations_ucl_lsoa.Rds")
piggyback::pb_upload("stations_ucl_lsoa.Rds")
```

```{r}
trips_df = fst::read_fst("data_clean_no_na_dates.fst")

stations = readRDS("stations_ucl_lsoa.Rds")
...
summary(stations$total_n_trips_start) # removed 2 stations
...
```{r}
tmap_mode("plot")
tm_shape(stations %>% select(operator_name, trips_per_year, trips_per_year_am, trips_per_year_pm, years_in_operation)) +
  ...
  tm_facets(ncol = 2)
tmap_save(tm = .Last.value, "figures/map-am-pm-peaks.png")
```
...
oas_lnd = oas_lnd[stations_buff, ] # works
...
lnd_osm_stations = lnd_osm[stations_buff, ]
plot(lnd_osm_stations$geometry)
lnd_osm_stations$type[is.na(lnd_osm_centroids$type)]
...
saveRDS(stations_pop, "stations_pop.Rds")
...
```{r}
stations = sf::read_sf("stations_pop.geojson")
...
g1
g2 = ggplot(stations %>% filter(! income_decile == "NA")) + geom_boxplot(aes(income_decile, trips_per_person_yr_am), draw_quantiles = c(0.25, 0.5, 0.75)) + scale_y_continuous(limits = c(0.1, 20)) + ylab("Number of trips per local resident (AM)")
g2
g3 = ggplot(stations %>% filter(! income_decile == "NA")) + geom_boxplot(aes(income_decile, trips_per_person_yr_pm), draw_quantiles = c(0.25, 0.5, 0.75)) + scale_y_continuous(limits = c(0.1, 20)) + ylab("Number of trips per local resident (PM)")
g3
library(patchwork)
g2 + g3 + plot_layout(ncol = 1)
# save as figures/income-decile-am-pm-boxplot.png
...
plot(stations_median_decile$index_of_multiple_deprivation_imd_decile_where_1_is_most_deprived_10_of_lso_as, stations$imd)
plot(stations["imd_decile_median_where_1_is_most_deprived"])
hist(oas$index_of_multiple_deprivation_imd_decile_where_1_is_most_deprived_10_of_lso_as, breaks = 0:11)
hist(stations$imd_decile_median_where_1_is_most_deprived, breaks = 0:11)
hist(stations$income_decile, breaks = 0:11)
plot(stations[5:ncol(stations)])
...
# hist(stations$imd)
hist(stations$imd_decile_median_where_1_is_most_deprived)
